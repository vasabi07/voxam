"""
olmOCR - PDF Extraction Test Script via DeepInfra

Usage:
    python test_olmocr.py <pdf_path>
...
# Pricing (per 1M tokens) - olmOCR
OLM_INPUT_PRICE = 0.09
OLM_OUTPUT_PRICE = 0.19
...
def extract_with_olmocr(images: list) -> tuple:
    """
    Send all pages to olmOCR via DeepInfra.
...
OUTPUT: Return ONLY the markdown study guide, no other text."""
    ...
    print(f"🚀 Sending {len(images)} pages to olmOCR...")
...
def calculate_cost(usage: dict) -> dict:
    ...
    input_cost = (input_tokens / 1_000_000) * OLM_INPUT_PRICE
    output_cost = (output_tokens / 1_000_000) * OLM_OUTPUT_PRICE
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python test_olmocr.py <pdf_path>")
        sys.exit(1)
    ...
    print(f"\n{'='*60}")
    print(f"🧪 olmOCR - PDF Extraction Test")
    print(f"{'='*60}")
    ...
    # Step 2: Extract with olmOCR
    markdown, usage = extract_with_olmocr(images)
    ...
    # Step 5: Save output
    output_path = Path(pdf_path).stem + "_olmocr_extracted.md"
