"""
Llama-4-Scout - PDF Extraction Test Script via DeepInfra

320k context window - handles large documents

Usage:
    python test_llama_scout.py <pdf_path>
"""
...
# Pricing (per 1M tokens) - Llama-4-Scout
INPUT_PRICE = 0.08
OUTPUT_PRICE = 0.30
...
def extract_with_llama(images: list) -> tuple:
    """
    Send all pages to Llama-4-Scout via DeepInfra.
...
OUTPUT: Return ONLY the markdown study guide, no other text."""
    ...
    print(f"🚀 Sending {len(images)} pages to Llama-4-Scout (320k context)...")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python test_llama_scout.py <pdf_path>")
        sys.exit(1)
    ...
    print(f"\n{'='*60}")
    print(f"🧪 Llama-4-Scout - PDF Extraction Test")
    print(f"{'='*60}")
    ...
    # Step 2: Extract with Llama-4-Scout
    markdown, usage = extract_with_llama(images)
    ...
    # Step 5: Save output
    output_path = Path(pdf_path).stem + "_llama_extracted.md"
