"""
LLM Latency Test - Nemotron Nano 3B (DeepInfra)
================================================
Simulates an exam agent scenario:
...
# DeepInfra client (OpenAI-compatible API)
client = OpenAI(
...
# Test student answers of varying quality
STUDENT_ANSWERS = {
...
def test_llm_streaming(student_answer: str, label: str):
...
def test_llm_non_streaming(student_answer: str, label: str):
    """Test non-streaming LLM response for comparison"""
...
def main():
    print("=" * 60)
    print("🧠 LLM LATENCY TEST - Nemotron Nano 8B (DeepInfra)")
    print("   Simulating voice exam agent scenario")
    print("=" * 60)
    
    results = {}
    
    for label, answer in STUDENT_ANSWERS.items():
        ttft, total = test_llm_streaming(answer, label)
    ...
    ns_total = test_llm_non_streaming(STUDENT_ANSWERS["correct"], "correct")
    if ns_total:
